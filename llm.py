...
def _limiter_for(provider: str) -> TokenBucket:
    return _DS_LIMITER if provider == "deepseek" else _GROQ_LIMITER
...
def _breaker_for(provider: str) -> CircuitBreaker:
    return _DS_BREAKER if provider == "deepseek" else _GROQ_BREAKER
...
def _route(prompt: str, lang: Optional[str], system: str) -> tuple[str, str, OpenAI]:
    ...
    """
    # Smart routing path (opt-in via AIM_SMART_ROUTING=1)
...
def ask_fast(prompt: str, lang: str = None) -> str:
    """Быстрый ответ — DeepSeek-chat (или Groq через роутер)."""
...
def ask_deep(prompt: str, system: str = "", lang: str = None) -> str:
    ...
    try:
        ...
        # DeepSeek R1 specific: reasoning_content holds the hidden chain-of-thought
...
def ask_long(prompt: str, system: str = "", lang: str = None,
    ...
    to LLM_MAX_TOKENS_LONG (131072 by default; tune via AIM_LLM_MAX_TOKENS_LONG).
    Falls back to ask() if DeepSeek key missing.
    """
    return ask(prompt, system=system, lang=lang,
