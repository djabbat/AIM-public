...
### Validation
- [ ] Build gold‑set: 200 CBCs with expert‑labelled syndromes (ground truth) — **N = TBD, not yet collected**
- [ ] Compute top‑1/top‑3 accuracy, calibration (ECE), red‑flag‑miss rate — **requires gold-set**
- [ ] Compare with human performance (Cabitza 2021, Goh 2024) — **requires gold-set**
...
|------|-------------|--------|------------|
| Reference ranges inaccurate for target population | Medium | High | Validate against local lab data; implement population-specific config |
| Gold set too small for reliable accuracy estimates | High | High | Plan for n=200 minimum; bootstrap confidence intervals |
| Pattern definitions miss rare syndromes | Medium | Medium | Implement data-driven clustering to discover new patterns |
| 5-zone hypothesis not clinically superior to 3-zone | Medium | Medium | Conduct head-to-head comparison in validation study |
| Low reproducibility due to missing code/data | Low | High | Enforce open science practices from Phase 0 |
| Integration with DiffDiagnosis fails | Low | Medium | Define clear API contract; test with mock data |

## Consortium / partners

- **Principal Investigator**: Dr. Jaba Tkemaladze (CEO, GLA)
- **Clinical advisors**: [TBD — list of haematologists]
- **Data providers**: [TBD — hospitals/labs for gold set collection]
- **Technical collaborators**: [TBD — Rust/Phoenix developers]
- **Funding**: [TBD — grant sources]

*Note: All partner names and affiliations are placeholders pending formal agreements.*
