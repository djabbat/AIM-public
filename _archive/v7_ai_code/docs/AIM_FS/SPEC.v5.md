```markdown
# AIM Filesystem Specification (AIM_FS) — v4 (fixed per peer review)

Дата: 2026-05-16 
Статус: DRAFT v4 — исправлен по peer review от 2026-05-14 
...
> - Исправлено написание `on_dep_expire` (было `"deprecate"` → `"deprecate"`). 

<!-- review_response: исправление опечатки принято. -->
...
<!-- review_response: materialized contradicts index оставлен как необходимый для детекции конфликтов на этапе propose; упрощение до full scan отложено на случай, если производительность станет проблемой. -->
...
- Рекурсия реализована через топологическую сортировку подграфа зависимостей глубиной до `max_decay_depth`. Циклы обнаруживаются и игнорируются (запись в event log). 

<!-- review_response: добавлено ограничение глубины для предотвращения дорогих обходов (CRITICAL 7). -->
...
5. **Таймаут (настраиваемый параметр `dispute_timeout_days` в конфиге, по умолчанию 7 дней):** 
 ...
 - **Поведение при таймауте:** запись, вызвавшая конфликт (новая), получает статус `superseded`, а старая остаётся `active`. Если конфликт между двумя существующими записями (например, при каскадном decay), выбирается запись с более высоким `confidence`; при равенстве — более старая остаётся `active`. 
...
<!-- review_response: при таймауте одна запись автоматически становится superseded, неконсистентность устранена (CRITICAL 3). -->
...
**Публичный API** — с дополнением: все мутирующие операции (`propose`, `approve`, `reject`, `undo`, `resolve_dispute`) принимают опциональный `idempotency_key` (ULID). Если ключ передан и операция уже была выполнена (ключ найден в логе idempotency), возвращается предыдущий результат без повторного выполнения. Если ключ не передан, операция выполняется как обычно (без идемпотентности). 
**Внутренние модули:**
...
- `decay.rs` — background sweeper с per-entity lock и каскадным обходом (топологическая сортировка, ограничение глубины 3).
...
- **Идемпотентность:** Phoenix генерирует `idempotency_key` (ULID) для каждого запроса к Port. При сетевой ошибке или таймауте клиент повторяет запрос с тем же ключом; Rust возвращает ранее сохранённый результат, если операция уже выполнена. 
- **Fault-tolerance:** при падении Rust-процесса orphan locks не возникает, т.к. flock автоматически снимается при закрытии всех fd ядром. Потеря сообщения обрабатывается через idempotency: если запрос не дошёл, клиент повторяет; если результат не был отправлен, повтор с тем же ключом вернёт предыдущий результат (операция уже записана в idempotency log). 
...
- **Дедупликация:** перед миграцией вычисляется SHA-256 содержимого каждого файла; дубликаты (одинаковый SHA и одинаковая схема и одинаковый scope) объединяются в одну запись с версиями. Файлы с разной схемой или разным scope считаются разными записями. 
...
<!-- review_response: дедупликация по (SHA, schema, scope) для корректности (CRITICAL 2). -->

---

## 11. Преимущества над Claude (сводная таблица)

| Lim. | Claude | AIM_FS (v4) | Как |
|------|--------|-------------|-----|
...
| L6 | Verify-on-use | TTL + decay sweeper + каскадный decay (топологическая сортировка, макс. глубина 3) | `decay.ttl_days` + background |
...
| L10 | Дубли молча | Conflict log с таймаутом, авто-выбор при таймауте, поддержка >2 записей | `_service/disputes/` + blocked proposals + superseded при таймауте |
...
| L15 | Не atomic | Atomic transactions + per-entity lock + batch + idempotency | prepare/commit через tmp + per-entity flock + idempotency key |

> **v4 (изменения по сравнению с peer review):** 
> - L2: materialized index оставлен, DuckDB отложен. 
> - L4: уточнено, что MVP уступает по скорости глобального поиска; гибкость по frontmatter остаётся преимуществом. 
> - L5: добавлен абсолютный лимит pending. 
> - L6: ограничена глубина каскадного decay (3). 
> - L10: при таймауте конфликта одна запись автоматически superseded. 
> - L15: добавлена идемпотентность через idempotency_key. 
> - L14: статус не изменился (только в планах). 
...
2. **DuckDB vs SQLite:** отложено; на MVP граф читается из links.jsonl + materialized contradicts index. *(RESOLVED)* 
...
10. **Concurrent writes:** per-entity lock + prepare/commit (секция 4.2). *(RESOLVED)* 
11. **Каскадный decay:** топологическая сортировка с ограничением глубины (секция 7). *(RESOLVED)* 
...
14. **Глобальные запросы:** ограничены frontmatter (раздел 5.1). *(RESOLVED)* 
15. **Undo-механизм для auto-approve:** окно отмены 24 часа с каскадом (раздел 4.3). *(RESOLVED)* 

**Новые открытые вопросы:** 
- Оптимизация materialized contradicts index для больших графов (возможный переход на Tantivy в Phase 5). 
...
- propose/approve/reject API с атомарностью и idempotency key; batch-операции 
...
**Phase 4 (2 дня):** Sharded INDEX.json + lazy loading; миграция legacy с дедупликацией по (SHA, schema, scope). 

**Phase 5 (4 дня):** Conflict resolution UI, таймауты (параметризованные), auto‑approve preferences, decay sweeper (топологическая сортировка с ограничением глубины, обнаружение циклов). 
...
**Конец draft v4. Исправлен по peer review от 2026-05-14.**
```