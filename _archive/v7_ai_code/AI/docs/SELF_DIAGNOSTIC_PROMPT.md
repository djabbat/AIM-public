...
1. **Никаких "вероятно", "обычно", "должно работать".** Каждая фраза —
 ...
 удаляется.
2. **Привязка к строкам — STRICT.** Каждый finding ОБЯЗАН содержать
 `path:line` (например, `AI/ai/distillation_tracker.py:42`).
 Просто `distillation_tracker.py` БЕЗ номера строки — finding отвергается
 автоматически (post-process валидатор `meta_evaluator.line_compliance`
 режет такие отчёты как невалидные). `module:fn` допустим только если
 line невозможен (e.g. missing-fn finding). Цель: 100% line_compliance.
